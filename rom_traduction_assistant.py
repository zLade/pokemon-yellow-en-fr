...
DEFAULT_MOVE_LABEL_CATALOG = ROOT / "locales" / "fr-FR" / "move_labels_two_line.csv"
DEFAULT_FRENCH_POINTER_VARIANTS = (
    ROOT / "locales" / "fr-FR" / "pointer_variants.csv"
)
...
def verified_dialogue_restoration_payloads(
    ...
    for ref, text in sorted(RESTORED_DIALOGUES.items()):
...
def command_build(args: argparse.Namespace) -> int:
    ...
    for row in rows:
        ...
        encoded = format_game_text(text, row.layout)
        conflicts = literal_slot_conflicts(text)
        if conflicts:
            preflight_errors.append(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    repacked.add_argument(
        ...
        help=(
            ...
            "utilise locales/fr-FR/move_labels_two_line.csv s'il existe."
        ),
    )
    repacked.add_argument(
        "--move-label-report",
    ...
    repacked.add_argument(
        ...
        help=(
            "Catalogue des sens français distincts qui partagent un "
            "payload dans la reconstruction anglaise 2015."
        ),
    )
