...
if ($IsolationRootDirectory) {
    $IsolationRootDirectory = [System.IO.Path]::GetFullPath(
        $IsolationRootDirectory
    )
}
foreach ($Path in @(
    $RomPath,
...
$IsolationRunDirectory = if ($IsolationRootDirectory) {
    Join-Path $IsolationRootDirectory $RunName
} else {
    $RunDirectory
}
$ControlOutput = Join-Path $RunDirectory '01-fresh-control'
$ControlIsolation = Join-Path `
    $IsolationRunDirectory `
    'isolation-fresh-control'
...
$SharedIsolation = Join-Path $IsolationRunDirectory 'isolation-shared'
...
[System.IO.Directory]::CreateDirectory($IsolationRunDirectory) | Out-Null
