-- Capture the three complete English 2.0 intro dialogue pages at the game's
-- real input prompt.  This is intentionally separate from the five-page
-- French probe: the reviewed English intro has three compact source entries.
...
local function capturePrompt(signature)
    ...
    local prefix = string.format(
        "english_intro_prompt_%02d_frame_%04d",
        promptCount,
        frames
    )
    ...
    print(string.format(
        "POKEMON_ENGLISH_INTRO_PROMPT_CAPTURE index=%d frame=%d " ..
        "pngBytes=%d",
        promptCount,
        frames,
        #screenshot
...
emu.addEventCallback(function()
    ...
    if promptCount >= 3 and not finished then
        finished = true
        print(string.format(
            "POKEMON_ENGLISH_INTRO_PROMPTS_PASS mapper=163 region=%s " ..
            "frames=%d nmi=%d prompts=%d",
    ...
    elseif frames == 3000 then
        writeBinary("english_intro_prompt_timeout.png", emu.takeScreenshot())
        writeBinary(
            "english_intro_prompt_timeout_oam_256.bin",
            memoryBytes(emu.memType.nesSpriteRam, 0x00, 0xFF)
        )
        writeBinary(
            "english_intro_prompt_timeout_nametable_4k.bin",
            memoryBytes(emu.memType.nesPpuDebug, 0x2000, 0x2FFF)
        )
        print(string.format(
            "POKEMON_ENGLISH_INTRO_PROMPTS_FAIL mapper=163 region=%s " ..
            "frames=%d nmi=%d prompts=%d",
            tostring(emu.getState()["region"]),
            frames,
            nmiCount,
