...
def build_manifest(
    *,
    ...
    for main_offset, variant in sorted(
        ...
        if reference in seen_references:
            raise ValueError(
                f"duplicate pointer-variant reference {_hex(reference)}"
            )
        seen_references.add(reference)
        final_target = allocations[reference]
        expected_payload = format_game_text(
            variant.text,
        ...
        if payload != expected_payload:
            raise ValueError(
                f"pointer-variant payload mismatch at {_hex(reference)}"
            )
        expected_word = cpu_addr_for_offset(final_target)
        actual_word = int.from_bytes(
            final_rom[reference:reference + 2],
        ...
        if actual_word != expected_word:
            raise ValueError(
                f"pointer variant {_hex(reference)} has wrong pointer word"
            )
        source_word = int.from_bytes(
            original[reference:reference + 2],
            "little",
        )
        ...
        if source_target is None:
            raise ValueError(
                f"pointer variant {_hex(reference)} has no source target"
            )
        records.append(
            {
                "kind": "pointer_variant",
                "reference": _hex(reference),
                "source_row": _hex(main_offset),
                "source_target": _hex(source_target),
                "final_target": _hex(final_target),
                "pair": pair_for_offset(reference),
                "cpu_address": f"0x{expected_word:04X}",
                "provenance": "pointer_variant_reviewed",
                "payload_length": len(payload),
                "payload_sha256": _sha256(payload),
            }
        )
    ...
    return {
        ...
        "summary": {
            ...
            "pointer_variants": len(pointer_variant_entries),
...
def validate_manifest(
    *,
    ...
    for index, record in enumerate(records):
        ...
        elif kind == "pointer_variant":
            pointer_variant_count += 1
            if provenance != "pointer_variant_reviewed":
                errors.append(f"{label}: invalid pointer variant provenance")
            if record.get("source_row") is None:
                errors.append(f"{label}: pointer variant source_row is missing")
            if record.get("source_target") is None:
                errors.append(
                    f"{label}: pointer variant source_target is missing"
                )
        else:
    ...
    if summary.get("pointer_variants") != pointer_variant_count:
        errors.append("summary pointer_variants differs")
