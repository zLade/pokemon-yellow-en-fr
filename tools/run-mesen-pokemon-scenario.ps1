...
try {
    ...
    while (-not $Process.HasExited -and (Get-Date) -lt $Deadline) {
        ...
        $Process.Refresh()
        if ($ExpectedMarker -and (Test-Path -LiteralPath $StdoutPath)) {
            $LiveStdout = Get-Content -LiteralPath $StdoutPath -Raw
            if ($LiveStdout -match [regex]::Escape($ExpectedMarker)) {
    ...
    if (-not $Process.HasExited) {
        Stop-Process -Id $Process.Id -Force
        $Process.WaitForExit()
        throw "Mesen scenario timed out after $TimeoutSeconds seconds."
    }

    $Process.Refresh()
    $ProcessExitCode = if ($ExpectedMarker -and $ExpectedMarkerObserved) { 0 } else { $Process.ExitCode }
    ...
    if ($Process.ExitCode -ne 0 -and -not $ExpectedMarkerObserved) {
