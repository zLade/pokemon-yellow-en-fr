...
Checks IPS provenance and builds the English localization from its catalog.
All source offsets include the iNES header.
...
def resolve_text_profile(
    ...
    """Resolve a CLI/profile value, defaulting to the English text profile."""
...
def default_move_label_catalog(
    ...
    candidate = ROOT / "translation" / "move_labels_two_line.csv"
...
def verified_dialogue_restoration_payloads(
    ...
    *,
    ...
    Callers must supply the complete English restoration catalogue explicitly.
    There is no implicit or French restoration corpus fallback.
    """
    profile = resolve_text_profile(text_profile)
...
def command_build_repacked(args: argparse.Namespace) -> int:
    ...
    # The English catalogue contains both MAIN and all 85 RESTORED records.
    restoration_texts = load_restoration_texts_csv(
        restoration_csv or args.csv,
        text_profile=text_profile,
    )
    rows = read_translation_csv(
        args.csv,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Build and validate the English Pokemon Yellow NES localization.'
    )
    ...
    build.add_argument(
        ...
        help="Text profile (default: en-US).",
    )
    build.add_argument("--csv", default=DEFAULT_CATALOG)
    build.add_argument("--input-rom", default=TRANSLATION_BASE_ROM)
    build.add_argument("--output-rom", default=FINAL_ROM)
    build.add_argument("--output-ips", default=FINAL_IPS)
    ...
    repointed.add_argument("--csv", default=DEFAULT_CATALOG)
    repointed.add_argument("--input-rom", default=TRANSLATION_BASE_ROM)
    repointed.add_argument("--output-rom", default=FINAL_ROM)
    repointed.add_argument("--output-ips", default=FINAL_IPS)
    ...
    repacked.add_argument("--csv", default=DEFAULT_CATALOG)
    repacked.add_argument(
        "--restorations-csv",
        default="",
        help=(
    ...
    repacked.add_argument("--output-rom", default=FINAL_ROM)
    repacked.add_argument("--output-ips", default=FINAL_IPS)
    repacked.add_argument("--fixed-overflow-output", default="build/en/fixed_overflow.csv")
    repacked.add_argument(
        "--move-labels-csv",
        default="",
        help=(
