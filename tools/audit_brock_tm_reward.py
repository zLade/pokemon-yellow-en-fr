...
def move_name_record_offset(rom: bytes, tm_number: int) -> int:
    if not 1 <= tm_number <= TM_COUNT:
        raise ValueError(f"TM number out of range : {tm_number}")
    ...
    if not MOVE_NAME_BANK_FILE_BASE <= target < MOVE_NAME_BANK_FILE_BASE + 0x8000:
        raise ValueError(
            f"CT{tm_number:02d}: move pointer outside bank 0x{pointer:04X}"
        )
    return target
...
def french_move_for_tm(rom: bytes, tm_number: int) -> str:
    ...
    try:
        ...
        raise ValueError(
            f"CT{tm_number:02d}: move 0x{target:06X} missing from script"
        ) from exc
...
def ascii_record(rom: bytes, offset: int) -> str:
    ...
    if any(value < 0x20 or value > 0x7E for value in payload):
        raise ValueError(f"non-ASCII record at 0x{offset:06X}")
...
def first_number(pattern: str, text: str, *, label: str) -> int:
    ...
    if match is None:
        raise ValueError(f"{label} missing from {text!r}")
...
def audit_brock_tm_reward() -> BrockTmAudit:
    ...
    for tm_number in (source_receipt_tm, source_explanation_tm):
        ...
        if source_entry != english_entry:
            raise ValueError(f"CT{tm_number:02d}: gameplay mapping changed")
    ...
    if source_explanation_tm != english_runtime_receipt_tm:
        raise ValueError("source explanation and English receipt disagree")
    if resolved_move != "Armure" or tm34_move == resolved_move:
        raise ValueError("the executable table does not distinguish CT34 from CT35")
    return BrockTmAudit(
...
def main() -> int:
    ...
    print(
        f"Resolved TM : CT{result.resolved_tm:02d} / {result.resolved_move}; "
        f"CT{result.source_receipt_tm:02d} teaches {result.tm34_move}."
    )
    print("Conclusion: the '34' in the Chinese receipt is a typo.")
    return 0
