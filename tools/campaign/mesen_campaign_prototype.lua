...
-- The full 4 KiB checksum also contains the hidden nametable left behind by
-- the introduction.  Translated intro text can change that hidden page while
-- the visible bedroom remains byte-for-byte identical.  Keep the historical
-- checksum, but also recognize the stable visible $2000 page.
local BEDROOM_VISIBLE_NAMETABLE = 0xCD7E1521
local DOWNSTAIRS_NAMETABLE = 0xE9D58C68
local OUTSIDE_NAMETABLE = 0xDD441740
local OUTSIDE_VISIBLE_NAMETABLE = 0xCBA23726
...
local LAB_VISIBLE_NAMETABLE = 0x746AFA1F
...
local cachedVisibleNametable = 0
...
local function visibleNametableChecksum()
    local hash = 0
    for address = 0x2000, 0x23FF do
        hash = (
            hash * 257 + emu.read(address, emu.memType.nesPpuDebug)
        ) % 0x100000000
    end
...
local function isBedroomNametable()
    return cachedNametable == BEDROOM_NAMETABLE
        or cachedVisibleNametable == BEDROOM_VISIBLE_NAMETABLE
end

local function isOutsideNametable()
    return cachedNametable == OUTSIDE_NAMETABLE
        or cachedVisibleNametable == OUTSIDE_VISIBLE_NAMETABLE
end

local function isLabNametable()
    return cachedNametable == LAB_NAMETABLE
        or cachedVisibleNametable == LAB_VISIBLE_NAMETABLE
end
...
local campaignJournalHook = rawget(
    _G,
    "POKEMON_CAMPAIGN_JOURNAL_HOOK"
)
if campaignJournalHook ~= nil and type(campaignJournalHook) ~= "function" then
    error("POKEMON_CAMPAIGN_JOURNAL_HOOK must be a function")
end
...
engine:add_state("take_bedroom_stairs", {
    ...
    success = function(ctx)
        ...
        if cachedNametable == DOWNSTAIRS_NAMETABLE
            or (cachedVisibleNametable == BEDROOM_VISIBLE_NAMETABLE
                and positionInRange(position, "x", 0xBE, 0xC2)
                and positionInRange(position, "y", 0x1E, 0x22))
        then
            ctx.input:abort()
            return true, position
        end
        if isBedroomNametable() and ctx.input:is_idle() then
...
engine:add_state("prove_outside_stable", {
    ...
    success = function(ctx)
        if not isOutsideNametable() then
            ctx.outside_candidate = nil
            ctx.outside_candidate_since = nil
            return false
        end
...
engine:add_state("walk_to_rival_trigger", {
    ...
    success = function(ctx, elapsed)
        local position = coordinates()
        if elapsed >= 30 and not isLabNametable() then
            ctx.input:abort()
...
engine:add_state("complete_rival_battle", {
    ...
    success = function(ctx, elapsed)
        local position = coordinates()
        local party = leadPartySummary()
        if elapsed >= 240 and not isLabNametable() then
        ...
        if not waitingForStableReturn
            and ctx.input:is_idle()
        then
            ctx.input:pulse("a", 3, 24, "advance_rival_battle")
        end
        return false
    end,
    timeout = 7200,
...
local campaignFinalizeHook = rawget(
    _G,
    "POKEMON_CAMPAIGN_FINALIZE_HOOK"
)
if campaignFinalizeHook ~= nil and type(campaignFinalizeHook) ~= "function" then
    error("POKEMON_CAMPAIGN_FINALIZE_HOOK must be a function")
end

local campaignInputTakeoverHook = rawget(
    _G,
    "POKEMON_CAMPAIGN_INPUT_TAKEOVER_HOOK"
)
if campaignInputTakeoverHook ~= nil
    and type(campaignInputTakeoverHook) ~= "function"
then
    error("POKEMON_CAMPAIGN_INPUT_TAKEOVER_HOOK must be a function")
end
...
emu.addEventCallback(function()
    ...
    if frame % CHECKSUM_SAMPLE_PERIOD == 0 then
        ...
        cachedVisibleNametable = visibleNametableChecksum()
    end
    ...
    finalized = true
    writeEvidence()
    if campaignFinalizeHook ~= nil then
        local hookOk, hookError = pcall(
            campaignFinalizeHook,
            status,
            context.endpoint,
            frame
        )
        if not hookOk then
            print(
                "POKEMON_CAMPAIGN_PROTOTYPE_FAIL frame=" ..
                tostring(frame) .. " state=finalize_hook reason=" ..
                tostring(hookError)
            )
            emu.stop(1)
            return
    ...
    if status ~= "completed" then
        ...
        print(string.format(
            "POKEMON_CAMPAIGN_PROTOTYPE_FAIL frame=%d state=%s reason=%s",
            frame,
