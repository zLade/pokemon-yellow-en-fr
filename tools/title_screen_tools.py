...
# Deux ecrans secondaires du menu joueur stockent eux aussi leurs libelles
# sous forme de tuiles 2 bpp reparties sur deux rangees.  Les mots anglais ne
# sont donc pas detectables comme chaines ASCII dans la ROM.
SAVING_SCREEN_CHR_FILE = 0x0299F5
SAVING_SCREEN_NT_FILE = 0x02A155
STATUS_SCREEN_CHR_FILE = 0x02A7C5
STATUS_SCREEN_NT_FILE = 0x02B4E5
...
FRENCH_BOTTOM_TITLE_TILE_IDS = (0x81, 0x82, 0x83, 0x7F, 0x84)
FRENCH_BOTTOM_TITLE_NT_ROW = 24
FRENCH_BOTTOM_TITLE_NT_COLUMN = 6
FRENCH_BOTTOM_TITLE_NT_WIDTH = 20
...
BOTTOM_TITLE_FONT_5X6 = {
    "J": ("11111", "00100", "00100", "00100", "10100", "01100"),
...
def bottom_title_letter_tile(letter: str) -> bytes:
    """Render one white letter in the original red/black bottom banner."""
    rows = BOTTOM_TITLE_FONT_5X6[letter]
    low = [0xFF] * 8
    high = [0xFF, 0xFF, 0, 0, 0, 0, 0xFF, 0xFF]
...
FRENCH_BOTTOM_TITLE_TILES = tuple(
    bottom_title_letter_tile(letter) for letter in "JAUNE"
)
FRENCH_BOTTOM_TITLE_TILEMAP = bytes(
    [0x8E] * 5
...
PLAYER_MENU_FONT_4X7 = {
    ...
    "É": ("0010", "1111", "1000", "1110", "1000", "1000", "1111"),
}
...
STATUS_SCREEN_LABELS = {
    "NAME": {
        "french": "NOM",
        "top": (0x7C, 0x7D, 0x7E, 0x7F),
        "bottom": (0x87, 0x88, 0x89, 0x8A),
        "row": 10,
        "column": 6,
        "original_sha256": (
            "aba9babef77a973915934a0de6a1f010"
            "5e76de4997d53d480177a74b0c5dd901"
        ),
    },
    "MONEY": {
        "french": "ARGENT",
        "top": (0x95, 0x96, 0x97, 0x98),
        "bottom": (0x9E, 0x9F, 0xA0, 0xA1),
        "row": 13,
        "column": 6,
        "original_sha256": (
            "e8d1defb9d6999203fb9fdfab319c1c"
            "b53f0b74cda743fc8f21a4e336b10b4d3"
        ),
    },
    "POKEDEX": {
        "french": "POKÉDEX",
        "top": (0xAC, 0xAD, 0xAE, 0xAF, 0x83, 0x8D),
        "bottom": (0xB5, 0xB6, 0xB7, 0xB8, 0x84, 0x8E),
        "row": 16,
        "column": 6,
        "original_sha256": (
            "38b287b7f1929ad85f0d059d4c69da4"
            "99c86f09ca088f4b26b5414608231daa7"
        ),
    },
}
...
SAVING_SCREEN_LABEL = {
    "french": "SAUVEGARDE",
    "top": (0x04, 0x05, 0x06, 0x07, 0x08, 0x09),
    "bottom": (0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F),
    "row": 5,
    "column": 4,
    "original_sha256": (
        "c78209f879ce951d0829f9b222b4fabd"
        "9a3e4b561d9462f6f8a92e1731a3cc8e"
    ),
}

STATUS_SCREEN_PLAYER_NAME = {
    "french": "Sacha",
    "source_top": (0x81, 0x82, 0x78),
    "source_bottom": (0x8B, 0x8C, 0x78),
    "target_top": (0x81, 0x82, 0xB1),
    "target_bottom": (0x78, 0x78, 0x78),
    "row": 10,
    "column": 12,
    "duplicate_blank_row": 16,
    "duplicate_blank_column": 19,
    "original_sha256": (
        "83dbf3ce5e5c1ac9b3e5be87ff36033a"
        "83ccda8cadf53e771961a03e4859ae8e"
    ),
}

STATUS_NAME_FONT_4X7 = {
    "S": ("0110", "1001", "1000", "0110", "0001", "1001", "0110"),
    "a": ("0000", "0000", "0110", "0001", "0111", "1001", "0111"),
    "c": ("0000", "0000", "0111", "1000", "1000", "1000", "0111"),
    "h": ("1000", "1000", "1000", "1110", "1001", "1001", "1001"),
}


def status_player_name_tiles(name: str) -> list[bytes]:
    """Render the French player name in a safe three-tile-wide slot."""
    missing = sorted(set(name) - set(STATUS_NAME_FONT_4X7))
    if missing:
        raise ValueError(
            f"Glyphe(s) nom joueur manquant(s): {', '.join(missing)}"
        )
    width = 24
    ...
    cursor_x = 0
    for letter in name:
        glyph = STATUS_NAME_FONT_4X7[letter]
        for glyph_y, row in enumerate(glyph):
            for glyph_x, bit in enumerate(row):
                if bit == "1":
                    glyph_pixels.append((cursor_x + glyph_x, 1 + glyph_y))
    ...
    for x, y in glyph_pixels:
        pixels[y][x] = 1
    return [
        encode_chr_tile([row[column * 8 : column * 8 + 8] for row in pixels])
...
def player_menu_word_tiles(
    ...
    *,
    ...
    for color in (background_color, foreground_color, shadow_color):
        if not 0 <= color <= 3:
            raise ValueError(f"Indice de palette CHR invalide: {color}")

    pixels = [[background_color for _ in range(width)] for _ in range(16)]
...
def patch_french_bottom_title_label(rom: bytearray) -> None:
    """Replace the bottom YELLOW banner with a centered JAUNE banner."""
    for tile_id, tile in zip(
        FRENCH_BOTTOM_TITLE_TILE_IDS,
    ...
    nt_offset = TITLE_NT_FILE + (
        FRENCH_BOTTOM_TITLE_NT_ROW * 32 + FRENCH_BOTTOM_TITLE_NT_COLUMN
    )
    rom[
        nt_offset : nt_offset + FRENCH_BOTTOM_TITLE_NT_WIDTH
    ] = FRENCH_BOTTOM_TITLE_TILEMAP
...
def _patch_two_row_graphical_labels(
    rom: bytearray,
    *,
    chr_file: int,
    nametable_file: int,
    ...
    nametable = bytes(rom[nametable_file : nametable_file + 32 * 30])
    if len(nametable) != 32 * 30:
        raise ValueError("ROM trop courte pour le tilemap de menu secondaire")

    for source_name, layout in labels.items():
        top = tuple(layout["top"])
        bottom = tuple(layout["bottom"])
        tile_ids = top + bottom
        row = int(layout["row"])
        ...
        generated = player_menu_word_tiles(
            str(layout["french"]),
            len(top),
        ...
        current_hash = hashlib.sha256(current).hexdigest()
        if (
            current_hash != str(layout["original_sha256"])
            and current != generated_block
        ):
            raise ValueError(
                f"Tuiles {source_name} inattendues: SHA-256={current_hash}"
            )
        for tile_id, tile in zip(tile_ids, generated, strict=True):
            offset = chr_file + tile_id * 16
            rom[offset : offset + 16] = tile


def patch_french_status_screen_labels(rom: bytearray) -> None:
    """Translate the player-card labels while preserving its geometry."""
    _patch_two_row_graphical_labels(
        rom,
        chr_file=STATUS_SCREEN_CHR_FILE,
        nametable_file=STATUS_SCREEN_NT_FILE,
        labels=STATUS_SCREEN_LABELS,
        background_color=0,
        foreground_color=1,
        shadow_color=3,
    )
    layout = STATUS_SCREEN_PLAYER_NAME
    nametable = bytes(
        rom[STATUS_SCREEN_NT_FILE : STATUS_SCREEN_NT_FILE + 32 * 30]
    )
    row = int(layout["row"])
    column = int(layout["column"])
    source_top = tuple(layout["source_top"])
    source_bottom = tuple(layout["source_bottom"])
    target_top = tuple(layout["target_top"])
    target_bottom = tuple(layout["target_bottom"])
    actual_top = tuple(nametable[row * 32 + column : row * 32 + column + 3])
    actual_bottom = tuple(
        nametable[(row + 1) * 32 + column : (row + 1) * 32 + column + 3]
    )
    blank_offset = (
        int(layout["duplicate_blank_row"]) * 32
        + int(layout["duplicate_blank_column"])
    )
    duplicate_blank = nametable[blank_offset]
    if (
        (actual_top, actual_bottom, duplicate_blank)
        not in (
            (source_top, source_bottom, 0xB1),
            (target_top, target_bottom, 0x78),
        )
    ):
        raise ValueError(
            "Tilemap du nom joueur inattendu: "
            f"haut={actual_top}, bas={actual_bottom}, "
            f"blanc={duplicate_blank:02X}"
        )

    generated = status_player_name_tiles(str(layout["french"]))
    tile_ids = target_top
    current = b"".join(
        rom[
            STATUS_SCREEN_CHR_FILE + tile_id * 16
            : STATUS_SCREEN_CHR_FILE + (tile_id + 1) * 16
        ]
        for tile_id in tile_ids
    )
    generated_block = b"".join(generated)
    ...
    if (
        ...
        raise ValueError(
            "Tuiles du nom joueur inattendues: "
            f"SHA-256={current_hash}"
        )
    for tile_id, tile in zip(tile_ids, generated, strict=True):
        offset = STATUS_SCREEN_CHR_FILE + tile_id * 16
        rom[offset : offset + 16] = tile
    nt_offset = STATUS_SCREEN_NT_FILE + row * 32 + column
    rom[nt_offset : nt_offset + 3] = bytes(target_top)
    rom[nt_offset + 32 : nt_offset + 35] = bytes(target_bottom)
    rom[STATUS_SCREEN_NT_FILE + blank_offset] = 0x78


def patch_french_saving_screen_label(rom: bytearray) -> None:
    """Replace SAVING with SAUVEGARDE and retain the three dot tiles."""
    _patch_two_row_graphical_labels(
        rom,
        chr_file=SAVING_SCREEN_CHR_FILE,
        nametable_file=SAVING_SCREEN_NT_FILE,
        labels={"SAVING": SAVING_SCREEN_LABEL},
        background_color=1,
        foreground_color=3,
        shadow_color=0,
    )
...
def command_patch_french_graphics(args: argparse.Namespace) -> None:
    ...
    patch_french_status_screen_labels(rom)
    patch_french_saving_screen_label(rom)
    patch_title_credits(rom)
    patch_french_bottom_title_label(rom)
    patched = bytes(rom)
    ...
    manifest = "\n".join(
        [
            ...
            *title_reference_manifest_lines,
            ...
            (
                "Patch fiche joueur: NAME -> NOM, MONEY -> ARGENT, "
                "POKEDEX -> POKÉDEX"
            ),
            "Patch sauvegarde: SAVING... -> SAUVEGARDE...",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    patch_english_title = sub.add_parser(
        "patch-english-title",
        help="reprend le vrai titre de 2015 English reference et ajoute les crédits",
    )
