...
local menuProfile =
    rawget(_G, "POKEMON_TITLE_MENU_PROFILE") or "fr-FR"
if menuProfile ~= "fr-FR" and menuProfile ~= "en-US" then
    error("unsupported title/menu profile: " .. tostring(menuProfile))
end
local menuLabel = menuProfile == "en-US" and "menu_en" or "menu_fr"
local PASS_MARKER = menuProfile == "en-US"
    and "TITLE_EN_MENU_EN_PASS"
    or "TITLE_EN_MENU_FR_PASS"
local FAIL_MARKER = menuProfile == "en-US"
    and "TITLE_EN_MENU_EN_FAIL"
    or "TITLE_EN_MENU_FR_FAIL"
local CAPTURE_MARKER = menuProfile == "en-US"
    and "TITLE_EN_MENU_EN_CAPTURE"
    or "TITLE_EN_MENU_FR_CAPTURE"
...
if menuProfile == "en-US" then
    -- The English 2.0 target uses the canonical yellow.nes title artwork.
    TITLE_TILES = {
        {
            id = 0x51,
            address = 0x0510,
            bytes = bytesFromHex("00020202010101000705050502020203"),
        },
        {
            id = 0x52,
            address = 0x0520,
            bytes = bytesFromHex("009A92923A121B00FF656D6DC5EDA4BF"),
        },
        {
            id = 0x53,
            address = 0x0530,
            bytes = bytesFromHex("00444A4A4A4A6400EFBBB5B5B5B59BFE"),
        },
        {
            id = 0x54,
            address = 0x0540,
            bytes = bytesFromHex("00A8A8A8F8505000FC54545404ACA8F8"),
        },
        {
            id = 0x55,
            address = 0x0550,
...
if menuProfile == "en-US" then
    -- Original English NEW/LOAD tiles from the pinned 2015 English ROM.
    MENU_TILES = {
        {
            id = 0x30,
            address = 0x1300,
        ...
        },
        {
            id = 0x31,
            address = 0x1310,
            bytes = bytesFromHex("003C66667E603E00003C66667E603E00"),
        },
        {
            id = 0x32,
            address = 0x1320,
            bytes = bytesFromHex("00626A6A6A6A3C0000626A6A6A6A3C00"),
        },
        {
            id = 0x40,
            address = 0x1400,
            bytes = bytesFromHex("0070707070707E000070707070707E00"),
        },
        {
            id = 0x41,
            address = 0x1410,
        ...
        },
        {
            id = 0x42,
            address = 0x1420,
            bytes = bytesFromHex("003C66667E666600003C66667E666600"),
        },
        {
            id = 0x43,
            address = 0x1430,
            bytes = bytesFromHex("007C666666667C00007C666666667C00"),
        },
    }
end
    ...
    if titleCaptured and
       ...
       renderedTitleContract() then
        ...
        if menuStableFrames >= 3 then
            ...
            if #failures ~= 0 then
                fail(menuLabel, failures, report, screenshot)
                return
            end
            if menuProfile == "en-US" then
                exportState("title_menu_en", screenshot, report)
            else
