...
EXPECTED_TEXTS = {
    0x0349E5: "Huile reçue !",
    0x0349F2: "Huile Max reçue !",
    0x030417: "Bravo ! ",
...
class NonPokedexTranslationCorrectionsTests(unittest.TestCase):
    def test_exact_reviewed_texts_and_layouts(self) -> None:
        ...
        self.assertEqual(len(EXPECTED_TEXTS), 45)
    ...
    def test_ether_receive_slots_match_the_executable_item_records(self) -> None:
        entries = {
            entry.offset: entry
            for entry in parse_patch_entries(
                apply_dialogue_inventory=False,
        ...
        expected = {
            0x031769: "Huile",
            # The live battle-bag renderer leaves seven cells before its
            # fixed quantity field.  Preserve the corrected item identity in
            # the compact form pinned by the 40-entry item-table audit.
            0x03176F: "HuilMax",
            0x031B0E: "000000Restaure10PP 1 Atq",
            0x031B29: "000Restaure ts PP 1 Atq",
            0x0349E5: "Huile reçue !",
            0x0349F2: "Huile Max reçue !",
        }
        for offset, text in expected.items():
            with self.subTest(offset=f"0x{offset:06X}"):
                self.assertEqual(entries[offset].text, text)
