-- Validate and export the translated in-game player menu.
--
...
-- verifies the live CHR-RAM checksums for OBJETS/SACHA/CS/SAUVER and checks
-- that the original tilemap geometry is still intact.
...
emu.addEventCallback(function()
    ...
    if chrChecksum ~= 0xA9254C19 then
        failures[#failures + 1] = string.format(
            "full CHR %08X != A9254C19",
            chrChecksum
        )
    end
    ...
    writeBinary("player_menu_fr_screen.png", screenshot)
    writeBinary(
        "player_menu_fr_chr_ram_8k.bin",
        memoryBytes(emu.memType.nesChrRam, 0x0000, 0x1FFF)
    )
    writeBinary(
        "player_menu_fr_nametable_4k.bin",
        memoryBytes(emu.memType.nesPpuDebug, 0x2000, 0x2FFF)
    )
    writeBinary(
        "player_menu_fr_palette_32.bin",
        memoryBytes(emu.memType.nesPpuDebug, 0x3F00, 0x3F1F)
    )
    writeText(
        "player_menu_fr_validation.txt",
        table.concat(report, "\n") .. "\n" ..
    ...
    if #failures == 0 then
        print(string.format(
            "POKEMON_PLAYER_MENU_FR_PASS mapper=163 region=%s " ..
        ...
        print(
            "POKEMON_PLAYER_MENU_FR_FAIL mapper=163 region=" ..
            tostring(state["region"]) .. " " ..
